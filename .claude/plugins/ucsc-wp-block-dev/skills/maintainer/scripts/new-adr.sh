...
#
# Usage:
#   bash new-adr.sh <skill> <mode> "<title>"   # ADR-NNN-skill-mode-mode-detail.md
#   bash new-adr.sh <slug> "<title>"           # legacy alias, ADR-NNN-slug.md
#
# Example:
#   bash new-adr.sh maintainer backlog "Track a maintainer backlog"
...
if [ $# -ge 3 ]; then
  ...
  TITLE="$3"
  DETAIL=$(norm "$TITLE")
  NAME_PART="${SKILL}-${MODE}-mode-${DETAIL}"
else
  NAME_PART=$(norm "$1")
  TITLE="$2"
fi
...
ADR_FILE="ADR-${next_str}-${NAME_PART}.md"
