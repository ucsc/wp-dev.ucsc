...
- **FSE Theme Routing Fallbacks:** The plugin uses `template_include` to manually override the theme for placeholder pages like `/directory/<cruzid>/` that do not exist as real pages. However, in Block (FSE) themes, doing this entirely drops the site header/footer. Real singular pages should rely on standard `the_content` filtering instead to preserve the theme chrome.
- **Content Replacement Buffers:** When using `ob_start()` inside `the_content` filters, the output buffer must be concatenated with the original `$content` (e.g., `return $content . ob_get_clean();`) rather than outright replacing it. This ensures that any blocks added via the FSE Site Editor on a placeholder page are not discarded.
- **Dependency-Free PHP Tests:** Avoid running `wp-env` or booting the entire WordPress suite for simple block logic. Instead, mock `ABSPATH` and essential functions, and test via the `php:8.1-cli` container, as seen in `ClassScheduleTest.php` and `CampusDirectoryTest.php`.
- **Mocking LDAP Constants:** When executing dependency-free tests inside lightweight `php:8.1-cli` containers, the `ldap` extension might not be present. If testing code like `CampusDirectoryAPI.php` that uses `LDAP_OPT_*` constants, explicitly define them in the test stub (e.g., `if (!defined('LDAP_OPT_TIMELIMIT')) define('LDAP_OPT_TIMELIMIT', 0);`) to prevent `Undefined constant` fatal errors.
