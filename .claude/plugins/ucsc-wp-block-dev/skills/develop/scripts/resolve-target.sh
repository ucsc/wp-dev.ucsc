...
# resolve-target.sh — deterministically resolve the block target from a path
...
# with block-target-check.sh (a shallow, maxdepth-1 check — never a tree scan).
#
# Output: one line "<slug> <repo> <path>" matching session-target.sh's format,
...
usage() {
  cat <<'EOF'
Usage: resolve-target.sh [path] [--persist]
...
Options:
  --persist   Also store the result via session-target.sh set (when a slug is
...
if [ -n "$blockpath" ]; then
  if ! bash "$HERE/block-target-check.sh" "$blockpath" >/dev/null 2>&1; then
    slug=""
    blockpath=""
  fi
fi
...
if [ "$PERSIST" -eq 1 ] && [ -n "$slug" ]; then
  bash "$HERE/session-target.sh" set "$slug" "$repo" "$blockpath" >/dev/null 2>&1 || true
fi
