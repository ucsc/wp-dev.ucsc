...
# plugin location through source_base.sh (no hardcoded paths), quotes every
...
usage() {
  cat <<'EOF'
Usage: inspect_block_layout.sh <plugin-slug-or-path>
...
(ADR-095). The argument is either:
  - a known plugin slug (e.g. ucsc-blocks, ucsc-gutenberg-blocks), resolved via
    source_base.sh plugin-dir <slug>, or
...
if [ -d "$arg" ]; then
  ...
  base="$(bash "$SCRIPT_DIR/source_base.sh" plugin-dir "$arg" 2>/dev/null || true)"
fi
