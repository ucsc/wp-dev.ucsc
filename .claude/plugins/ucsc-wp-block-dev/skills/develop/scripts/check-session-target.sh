...
usage() {
  cat <<EOF
Usage: check-session-target.sh [show | --help]

Report the session block target in one call (ADR-093/ADR-094):
  - confirms session-target.sh and block-target-check.sh are present
  - prints the persisted target ("<slug> <repo> <path>", or a notice if unset)
  - validates the persisted filesystem path with block-target-check.sh when one
    is recorded

  show   Print the source of the target-resolution scripts (session-target.sh,
         block-target-check.sh) instead of running the checks — a safe single
...
case "${1:-}" in
  --help|-h|help)
    ...
    ;;
  show)
    for f in session-target.sh block-target-check.sh; do
      echo "===== $f ====="
...
ls -1 \
  "$SCRIPT_DIR/session-target.sh" \
  "$SCRIPT_DIR/block-target-check.sh"

echo "=== persisted session target (get) ==="
target="$(bash "$SCRIPT_DIR/session-target.sh" get || true)"
if [ -z "$target" ]; then
  echo "(no persisted target — resolve one per ADR-093 before proceeding)"
else
  echo "$target"
  target_path="$(bash "$SCRIPT_DIR/session-target.sh" dir || true)"
  if [ -n "$target_path" ]; then
    echo "=== validate persisted path is a real block ==="
    bash "$SCRIPT_DIR/block-target-check.sh" "$target_path" || \
