...
# wp-eval.sh — run a bundled PHP file inside the wpcli container via wp-cli over
...
# (block-doctor.sh, wp_facts.sh, list-blocks.sh-style helpers) call this.
#
# Usage:
#   wp-eval.sh <php-file> [KEY=VAL ...]
#
# Each KEY=VAL is forwarded into the container as an env var the PHP can read via
# getenv(). Example:
#   wp-eval.sh helpers/block-doctor.php DOCTOR_BLOCK=ucscblocks/classschedule
#
