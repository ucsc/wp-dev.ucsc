...
# seed-events-cache.sh — seed the ucsc/events transient so the block renders
# offline. The PHP (helpers/seed-events-cache.php) runs in the container via
# wp-cli over STDIN — no host PHP, no inline PHP in a shell string.
set -uo pipefail

HERE="$(cd "$(dirname "$0")" && pwd)"
PHP="$HERE/helpers/seed-events-cache.php"
SOURCE_BASE="$HERE/../develop/scripts/source-base.sh"

case "${1:-}" in
  --help|-h|help) echo "Usage: seed-events-cache.sh   # seed ucsc/events transient"; exit 0 ;;
esac
