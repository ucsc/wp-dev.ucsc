...
# list-blocks.sh — list every `ucsc/*` block registered in the RUNNING
...
# lives in helpers/list-blocks.php and is piped to wp-cli over STDIN, so no PHP
# is ever embedded in a shell string (ADR-095).
set -uo pipefail

HERE="$(cd "$(dirname "$0")" && pwd)"
PHP="$HERE/helpers/list-blocks.php"
SOURCE_BASE="$HERE/../develop/scripts/source-base.sh"

usage() {
  cat <<'EOF'
Usage: list-blocks.sh [--help]

List every ucsc/* block registered in the RUNNING wp-dev.ucsc WordPress, one per
line. Reads the live WP_Block_Type_Registry through wp-cli (helpers/list-blocks.php
