...
date: 2026-06-17
supersedes: []
related: ["ADR-003", "ADR-058", "ADR-064"]
---
...
ADR-003 established lean token spend as a principle. ADR-058 applied it to the
`hub` skill by mandating a static inventory (no filesystem scan, no agent
spawn). ADR-064 protected `validate` and `review-skills` from running as part
