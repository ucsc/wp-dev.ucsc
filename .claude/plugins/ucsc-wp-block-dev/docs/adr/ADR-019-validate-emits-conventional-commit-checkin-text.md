...
This output is the closing step of test mode; it does not gate the run and is skipped when no coverage changed (a plain test execution). Jira remains optional per [ADR-008](ADR-008-develop-prefer-jira-id-for-fix-and-develop.md) — when a Jira key is known it is referenced in the footer.
