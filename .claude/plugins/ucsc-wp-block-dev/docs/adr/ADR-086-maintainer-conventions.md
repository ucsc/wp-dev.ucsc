...
`ADR-086-maintainer-conventions.md`. Legacy hyphenated three-digit files are
renamed to the same four-digit underscore convention. `new-adr.sh` is updated
...
`check-adr-implements.py` runs two checks:
