...
ADR-067 introduced `sync_inventory.sh` for inventory consistency, but its
...
Extend `skills/maintainer/scripts/sync_inventory.sh` so:
...
- **Positive:** Stale skill names are caught by `sync_inventory.sh --check`
  and the existing pytest tests instead of through runtime confusion.
- **Negative:** `sync_inventory.sh --write` now edits a repository-root file
