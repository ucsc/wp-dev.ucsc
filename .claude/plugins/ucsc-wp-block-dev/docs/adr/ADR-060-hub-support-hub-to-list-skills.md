...
3. `:hub` stays token-frugal (ADR-003): a static inventory, not a scan.
...
1. **Repository detection is allowed, scans are not.** `:hub` may inspect the
   ...
   (`resolve-target.sh`), not the filesystem scan that decision point 3 of this
   ADR forbids; that prohibition still stands for building the inventory.
   "Offer" means **list** targets as information — the block target is optional
   for `:hub`, which never prompts for one or blocks the inventory on a selection
   (clarified 2026-06-24). The list is **not** scoped to the cwd repo: `:hub`
   lists targets for every known block repo that is **present on disk** (both
   `ucsc-blocks` and `ucsc-gutenberg-blocks` when both exist), and omits any repo
   that is absent. `:hub` sets the session target only when one is passed
   (point 2) or a single slug is unambiguously inferred from the cwd.
2. **A passed target is validated before adoption.** When the user runs
   `:hub <block>`, validate it with `block-target-check.sh` and resolve its repo
...
3. **`:hub` sets the session target but still does not route.** Persisting the
   target via `session-target.sh set <slug> <repo> <path>` lets later skills
